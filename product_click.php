...
$stmt = $connectDB->prepare('SELECT pro_name, pro_part, pro_part_img FROM product WHERE pro_name = :product_name');
$stmt->execute(['product_name' => $product_name]);
$pro = $stmt->fetchAll(PDO::FETCH_ASSOC);

// If no products are found, handle the error
if (count($pro) === 0) {
    echo "<div style='color: white; text-align: center; margin-top: 100px;'><h2>No products found for: " . htmlspecialchars($product_name) . "</h2></div>";
    exit();
}

// If no product is found, handle the error
if (!$pro) {
    echo "<div style='color: white; text-align: center; margin-top: 100px;'><h2>Product not found.</h2></div>";
    exit();
}
?>
...
<body>
    ...
    <div class="page-wrapper container">
        ...
        <!-- Parts Grid (Using modern Bootstrap wrapping instead of PHP row breaks) -->
        <div class="row">
            <?php foreach ($pro as $index => $product): ?>
                <div class="col-lg-4 col-md-6 col-sm-12 mb-4">
                    <a href="part_click.php?part=<?php echo urlencode($product['pro_part']); ?>&name=<?php echo urlencode($product['pro_name']); ?>" class="part-card-glass">
                        
                        <!-- Product Image -->
                        <img src="<?php echo htmlspecialchars($product['pro_part_img']); ?>" alt="<?php echo htmlspecialchars($product['pro_name']); ?>">
                        
                        <!-- Bottom Gradient Overlay -->
                        <div class="part-overlay">
                            <p><?php echo htmlspecialchars($product['pro_part']); ?></p>
                        </div>
